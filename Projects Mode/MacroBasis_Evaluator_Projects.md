> **PROJECTS-MODE COPY (v1.0, 21 Aug 2026).** Everything below this banner is byte-identical to
> `MacroBasis_Evaluator.md` at the repo root, so the two never drift. Three lines below name repo artifacts that do not exist in a
> Project; read them as follows:
> - **"engine-inserted Status Ball oval"** and **"the engine strips one if present"** (check family 2):
>   in Projects mode you write the status word and the deciding sentence as plain text, with no typed
>   bullet character. The Co-Op applies the coloured oval when they paste it in. The rule that matters
>   is unchanged: never a literal "Macro take:" label, and the read is woven in as a clause.
> - **"`runs/YYYY-MM-DD/triage.md`"** (check family 4): read as the confirmed Phase 4 triage table you
>   printed in this chat and will file as Appendix C. Diff against that table, not against memory.
> - **"`_canonical.reference_topics` in `engine/content_schema.json`"** (check family 7): read as
>   section 1 of `MacroBasis_Content_Spec.md`, which inlines the same labels.
>
