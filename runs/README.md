...
| `triage.md` | Phase 4 | Confirmed triage table (provenance column; every daily candidate included or discarded). Updated after the Co-Op's confirmation — this is the record Phase 7 reconciles against |
| `self_audit.md` | Phase 8 | Checklist confirmations + the **run manifest**: window, files read, thread counts, lights, divergences carried, and `edits_after_review:` (the Co-Op fills after their hand pass — the minimal-edits KPI, tracked week over week) |
